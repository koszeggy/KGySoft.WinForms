...
namespace KGySoft.WinForms.UnitTests
{
    [TestFixture]
    public class ResTests
    {
        ...
        [SuppressMessage("ReSharper", "GenericEnumeratorNotDisposed", Justification = "False alarm, these enumerators are not disposable")]
        private static void CheckCoverage(HashSet<string> obtainedMembers)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                // key is now not recognized in Res members. This is alright if it is an enum value for the Res.Get<TEnum> method.
                if (key.Contains('.'))
                {
                    string[] parts = key.Split('.');
                    Type? enumType = typeof(WinFormsModule).Assembly.GetTypes().FirstOrDefault(t => t.Name == parts[0]);
                    if (enumType?.IsEnum == true && Enum.IsDefined(enumType, parts[1]))
                        continue;
                }
