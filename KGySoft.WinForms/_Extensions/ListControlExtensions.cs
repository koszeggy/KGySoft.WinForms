...
namespace KGySoft.WinForms
{
    ...
    public static class ListControlExtensions
    {
        ...
        public static void LoadFrom(this ListControl control, DataTable dataTable, string valueMember, string displayMember, bool translateNames, string? distinctionPostfix, bool sortByDisplayedValues, SelectionPlusItems plusItems)
        {
            ...
            if (dataTable == null)
                throw new ArgumentNullException(nameof(dataTable), PublicResources.ArgumentNull);
            int indDisplay = dataTable.Columns.IndexOf(displayMember);
            if (indDisplay == -1)
                throw new ArgumentException(Language.Translate("Display member column not found"), nameof(displayMember));
            int indValue = dataTable.Columns.IndexOf(valueMember);
            if (indValue == -1)
                throw new ArgumentException(Language.Translate("Value member column not found"), nameof(valueMember));
            ...
            if (translateNames && colDisplay.DataType == typeof(string))
            {
                foreach (DataRow dr in dataTable.Rows)
                {
                    ...
                    if (dr[colDisplay] is string s)
                        dr[colDisplay] = Language.Translate(s + (!String.IsNullOrEmpty(distinctionPostfix) ? Language.DistinctionSeparator + distinctionPostfix : String.Empty));
                }
            }
            ...
            if ((plusItems & SelectionPlusItems.ItemNone) != 0)
            {
                ...
                dr[colValue] = ChangeType(colValue.DataType, ControlExtensions.NoneSelectedValue);
                dr[colDisplay] = Language.Translate(ControlExtensions.NoneSelectedText);
                dataTable.Rows.InsertAt(dr, 0);
            }
            if ((plusItems & SelectionPlusItems.ItemAll) != 0)
            {
                DataRow dr = dataTable.NewRow();
                dr[colValue] = ChangeType(colValue.DataType, ControlExtensions.AllSelectedValue);
                dr[colDisplay] = Language.Translate(ControlExtensions.AllSelectedText);
                dataTable.Rows.InsertAt(dr, 0);
            }
            if ((plusItems & SelectionPlusItems.ItemNotSelected) != 0)
            {
                ...
                if (!colValue.DataType.IsValueType)
                    ...
                    dr[colValue] = ChangeType(colValue.DataType, ControlExtensions.NotSelectedValue);
                dr[colDisplay] = Language.Translate(ControlExtensions.NotSelectedText);
                dataTable.Rows.InsertAt(dr, 0);
            }
        ...
        public static void LoadFrom(this ListControl control, Type enumType, Type? valueMemberType, bool translateNames, string? distinctionPostfix, bool sortByDisplayedValues, SelectionPlusItems plusItems)
        {
            if (control == null)
                throw new ArgumentNullException(nameof(control), PublicResources.ArgumentNull);
            if (enumType == null)
                throw new ArgumentNullException(nameof(enumType), PublicResources.ArgumentNull);
            if (!enumType.IsEnum)
                throw new ArgumentException(Language.Translate("Specified type is not an enum type"), nameof(enumType));

            valueMemberType ??= enumType;
            if (!typeof(IConvertible).IsAssignableFrom(valueMemberType))
                throw new ArgumentException(Language.Translate("Specified type must is not an IConvertible type"), nameof(valueMemberType));

            // adding items and translating
            Array values = Enum.GetValues(enumType);
            var result = new CircularList<KeyValuePair<object, string?>>(values.Length + 3);
            ...
            if ((plusItems & SelectionPlusItems.ItemNone) != 0)
            {
                object key = ChangeType(valueMemberType, ControlExtensions.NoneSelectedValue);
                string value = Language.Translate(ControlExtensions.NoneSelectedText);
                result.Insert(0, new KeyValuePair<object, string?>(key, value));
            }
            if ((plusItems & SelectionPlusItems.ItemAll) != 0)
            {
                object key = ChangeType(valueMemberType, ControlExtensions.AllSelectedValue);
                string value = Language.Translate(ControlExtensions.AllSelectedText);
                result.Insert(0, new KeyValuePair<object, string?>(key, value));
            }
            if ((plusItems & SelectionPlusItems.ItemNotSelected) != 0)
            {
                object key = !valueMemberType.IsValueType ? DBNull.Value : ChangeType(valueMemberType, ControlExtensions.NotSelectedValue);
                string value = Language.Translate(ControlExtensions.NotSelectedText);
                result.Insert(0, new KeyValuePair<object, string?>(key, value));
            }
        ...
        public static void LoadFrom<T>(this ListControl control, IEnumerable<T> collection, string valueMember, string displayMember, bool translateNames, string? distinctionPostfix, bool sortByDisplayedValues, SelectionPlusItems plusItems)
        {
            if (control == null)
                throw new ArgumentNullException(nameof(control), PublicResources.ArgumentNull);
            if (collection == null)
                throw new ArgumentNullException(nameof(collection), PublicResources.ArgumentNull);
            if (valueMember == null)
                throw new ArgumentNullException(nameof(valueMember), PublicResources.ArgumentNull);
            if (displayMember == null)
                throw new ArgumentNullException(nameof(displayMember), PublicResources.ArgumentNull);
            ...
            if (propValue == null)
                throw new ArgumentException(Language.Translate("Value member instance property not found"), nameof(valueMember));
            ...
            if (propDisplay == null)
                throw new ArgumentException(Language.Translate("Display member instance property not found"), nameof(displayMember));
            ...
            if ((plusItems & SelectionPlusItems.ItemNone) != 0)
            {
                object key = ChangeType(propValue.PropertyType, ControlExtensions.NoneSelectedValue);
                string value = Language.Translate(ControlExtensions.NoneSelectedText);
                result.Insert(0, new KeyValuePair<object, string>(key, value));
            }
            if ((plusItems & SelectionPlusItems.ItemAll) != 0)
            {
                object key = ChangeType(propValue.PropertyType, ControlExtensions.AllSelectedValue);
                string value = Language.Translate(ControlExtensions.AllSelectedText);
                result.Insert(0, new KeyValuePair<object, string>(key, value));
            }
            if ((plusItems & SelectionPlusItems.ItemNotSelected) != 0)
            {
                object key = !propValue.PropertyType.IsValueType ? DBNull.Value : ChangeType(propValue.PropertyType, ControlExtensions.NotSelectedValue);
                string value = Language.Translate(ControlExtensions.NotSelectedText);
                result.Insert(0, new KeyValuePair<object, string>(key, value));
            }
