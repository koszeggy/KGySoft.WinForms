...
namespace KGySoft.Controls
{
    internal sealed class RawIcon : IDisposable
    {
        ...
        private sealed class RawIconImage : IDisposable
        {
            ...
            internal int RawBpp
            {
                get { return isPng ? 32 : bmpHeader.biBitCount; }
            }
            ...
            /// From raw data
            /// </summary>
            internal RawIconImage(byte[] rawData, int bitCount)
            {
                ...
                rawColor = new byte[stride * (bmpHeader.biHeight >> 1)]; // height is doubled because of mask
                Buffer.BlockCopy(rawData, offset, rawColor, 0, rawColor.Length);
                offset += rawColor.Length;

                // mask image (AND)
                stride = ((bmpHeader.biWidth + 31) & ~31) >> 3;
                rawMask = new byte[stride * (bmpHeader.biHeight >> 1)];
            ...
            private static void FlipImageY(Bitmap bitmap)
            {
                ...
                //    for (int i = 0; i < bitmap.Height / 2; i++)
                //    {
                //        Kernel32.CopyMemory(lptmpbuffer, pixelPtr + (i * bitmapData.Stride), bitmapData.Stride);
                //        Kernel32.CopyMemory(pixelPtr + (i * bitmapData.Stride), pixelPtr + (((bitmap.Height - 1) - i) * bitmapData.Stride), bitmapData.Stride);
                //        Kernel32.CopyMemory(pixelPtr + (((bitmap.Height - 1) - i) * bitmapData.Stride), lptmpbuffer, bitmapData.Stride);
                //    }
                //}
            ...
            private void AssureRawFormatGenerated()
            {
                ...
                try
                {
                    ...
                    int strideC = Math.Abs(dataColor.Stride);
                    int strideB = Math.Abs(dataMask.Stride);

                    // If the image bpp is less than 32, transparent color cannot has transparency
                    if (bpp < 32)
                    ...
                    for (int y = 0; y < dataColor.Height; y++)
                    {
                        ...
                        for (int x = 0; x < dataColor.Width; x++)
                        {
                            ...
                            {
                                case 1:
                                    rawMask[(x >> 3) + posCY] = rawColor[(x >> 3) + posCY];
                                    break;
                                case 4:
                                    color = rawColor[(x >> 1) + posCY];
                                    paletteColor = palette[(x & 1) == 0 ? color >> 4 : color & 0x0F];
                                    if (paletteColor.EqualsWithColor(transparentColor))
                                    {
                                        rawMask[(x >> 3) + posBY] |= (byte)(0x80 >> (x & 7));
                                        rawColor[(x >> 1) + posCY] &= (byte)((x & 1) == 0 ? 0x0F : 0xF0);
                                    }
                                    break;
                                case 8:
                                    color = rawColor[x + posCY];
                                ...
                                case 24:
                                    int posCX = x * 3;
                                    Color pixelColor = Color.FromArgb(0, rawColor[posCX + posCY + 0],
                                        rawColor[posCX + posCY + 1],
                                        rawColor[posCX + posCY + 2]);
                                ...
                                case 32:
                                    ...
                                    {
                                        if (rawColor[(x << 2) + posCY + 3] == 0)
                                            rawMask[(x >> 3) + posBY] |= (byte)(0x80 >> (x & 7));
                                    }
                                    else
                                    {
                                        if (transparentColor != Color.Empty &&
                                            rawColor[(x << 2) + posCY + 0] == transparentColor.B &&
                                            rawColor[(x << 2) + posCY + 1] == transparentColor.G &&
                                            rawColor[(x << 2) + posCY + 2] == transparentColor.R)
                                        {
                                            rawMask[(x >> 3) + posBY] |= (byte)(0x80 >> (x & 7));
                                            rawColor[(x << 2) + posCY + 0] = 0;
                                            rawColor[(x << 2) + posCY + 1] = 0;
                                            rawColor[(x << 2) + posCY + 2] = 0;
                                        }
                                        else
                                        {
                                            rawColor[(x << 2) + posCY + 3] = 255;
                                        }
                                    }
                                    break;
                            }
            ...
            private void AssureBitmapsGenerated(bool isCompositRequired)
            {
                ...
                if (isPng)
                {
                    ...
                    {
                        if (dirEntryBitCountPng == 0 || dirEntryBitCountPng == 32)
                            bmpColor = result;
                        else
                        {
                            ...
                            if (dirEntryBitCountPng <= 8)
                                paletteBmpColor = bmpColor.GetColors(1 << dirEntryBitCountPng);
        ...
        private void Load(BinaryReader br, Size? size, int? bpp, int? index)
        {
            ...
            for (int i = 0; i < iconDir.idCount; i++)
            {
                ...
                if ((!size.HasValue || ((reqSize.Width == entry.bWidth || reqSize.Width == 256 && entry.bWidth == 0)
                    && (reqSize.Height == entry.bHeight || reqSize.Height == 256 && entry.bHeight == 0)))
                    && (!bpp.HasValue || entry.wBitCount == 0 || reqBpp == entry.wBitCount))
                {
                    br.BaseStream.Position = entry.dwImageOffset;
                    RawIconImage image = new RawIconImage(br.ReadBytes((int)entry.dwBytesInRes), entry.wBitCount);
